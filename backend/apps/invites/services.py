...
class InviteService:
    ...
    def create_invite(project, email, role, invited_by, product_ids=None):
        ...
        if role == "producer" and product_ids:
            for product_id in product_ids:
                InviteProductAccess.objects.create(
                    invite=invite,
        ...
        if invite.role == "producer":
            ...
            for invite_product in invite_products:
                ProducerProductAccess.objects.create(
                    ...
                    product=invite_product.product
                )
    ...
    def get_pending_invites(project):
        ...
        return ProjectInvite.objects.filter(
            project=project,
            is_accepted=False
        ).select_related('invited_by').prefetch_related('product_access__product')
    ...
    def cancel_invite(invite_id, user):
        ...
        if not user_project_role or (user_project_role.role != "owner" and invite.invited_by != user):
